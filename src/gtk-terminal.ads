...
package Gtk.Terminal is
   ...
   procedure Scroll_Down(number_of_lines : in positive; 
      ...
      -- if possible.  For avoidance of doubt, if it is not possible, it does
      -- not insert any new line at the end.
   procedure Scroll_Up (number_of_lines : in positive; 
                        for_buffer :  access Gtk_Terminal_Buffer_Record'Class; 
                        starting_from :in out Gtk.Text_Iter.Gtk_Text_Iter);
      -- Scroll up the specified number of lines between scroll_region_top and
      -- scroll_region_bottom, making sure that lines that scroll below the
      -- scroll region are discarded and that the original lines outside of the
      -- scroll region are protected. If the scroll regions are undefined (i.e.
      -- set to 0), then this procedure does nothing other than go up a line if
      -- possible.  For avoidance of doubt, if it is not possible, it does not
      -- insert any new line at the start.
